...
module BS
  module JWT
    ...
    mattr_accessor :auth0_domain, :auth0_client, :jwks_endpoint, :jwks_key
    ...
    class << self
      ...
      def update_jwks
        check_config
        @jwks_key = JSON::JWK::Set.new(
          JSON.parse(fetch_jwks)
        )
      end

      def jwks_key
        super || update_jwks || raise(KeyMissing, 'Fetching JWKS key failed')
      end

      private

      def check_config
        %i[auth0_domain auth0_client jwks_endpoint].each do |key|
          val = send(key)
